...
function App() {
  ...
  return (
    <div className="min-h-screen bg-surface-950 text-white overflow-x-hidden selection:bg-brand-primary/30 flex flex-col font-sans">
      ...
      <nav className="fixed top-0 w-full z-50 glass border-b border-white/5 py-4 px-6 md:px-12 flex justify-between items-center text-sm font-medium">
        ...
        <div className="hidden md:flex gap-8 text-white/40">
          <button onClick={() => setStep('discover')} className={cn("hover:text-white transition-all text-xs font-black uppercase tracking-widest", step === 'discover' && "text-white")}>Explore</button>
          {user && <button onClick={() => setStep('dashboard')} className={cn("hover:text-white transition-all text-xs font-black uppercase tracking-widest", step === 'dashboard' && "text-white")}>Hub</button>}
          <a href="#" className="hover:text-white transition-all text-xs font-black uppercase tracking-widest">About</a>
        </div>
          ...
          {user ? (
            <div className="flex items-center gap-4">
              <button 
                ...
                className="hidden md:flex items-center gap-2 px-4 py-2 bg-brand-primary rounded-xl hover:bg-brand-primary/80 transition-all text-[10px] font-black uppercase tracking-widest shadow-lg shadow-brand-primary/20"
              >
            ...
            <button 
              ...
              className="bg-brand-primary px-6 py-2.5 rounded-full text-xs font-black uppercase tracking-widest hover:bg-brand-primary/80 transition-all active:scale-95 shadow-lg shadow-brand-primary/20"
            >
              Start
            </button>
          )}
            ...
            <div className="max-w-4xl mx-auto text-center px-6">
              ...
              <div className="flex flex-col md:flex-row gap-4 justify-center items-center">
                <button 
                  ...
                  className="w-full md:w-auto group bg-white text-black px-12 py-5 rounded-3xl font-black text-sm flex items-center justify-center gap-2 hover:bg-white/90 transition-all active:scale-95 shadow-22xl"
                >
                  Find My Group <ChevronRight className="w-5 h-5" />
                </button>
                <button onClick={() => setStep('discover')} className="w-full md:w-auto bg-white/5 border border-white/10 px-12 py-5 rounded-3xl font-black text-sm hover:bg-white/10 transition-all backdrop-blur-md">Explore Mapping</button>
              </div>
            </div>
          </motion.main>
        )}

        {step === 'dashboard' && <motion.main key="dashboard" variants={pageVariants} initial="initial" animate="enter" exit="exit" className="flex-1 pb-24 md:pb-0"><Dashboard user={user} onSelectGroup={handleGroupSelect} /></motion.main>}
        {step === 'discover' && <motion.main key="discover" variants={pageVariants} initial="initial" animate="enter" exit="exit" className="flex-1 pb-24 md:pb-0"><Discover onSelectGroup={handleGroupSelect} /></motion.main>}
        {step === 'groupDetail' && selectedGroup && <motion.main key="detail" variants={pageVariants} initial="initial" animate="enter" exit="exit" className="flex-1 pb-24 md:pb-0"><GroupDetail group={selectedGroup} user={user} onBack={() => setStep('discover')} /></motion.main>}
        {step === 'createGroup' && <motion.main key="create" variants={pageVariants} initial="initial" animate="enter" exit="exit" className="flex-1 pb-24 md:pb-0"><CreateGroup userId={user?.id} onBack={() => setStep('dashboard')} onSuccess={(newGroup) => { setSelectedGroup(newGroup); setStep('groupDetail'); }} /></motion.main>}
      </AnimatePresence>
